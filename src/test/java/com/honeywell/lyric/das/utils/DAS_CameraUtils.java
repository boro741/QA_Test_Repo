...
public class DAS_CameraUtils {
	...
	public static boolean selectDASCameraSolutionCard(TestCases testCase,TestCaseInputs inputs){
		...
		Keyword.ReportStep_Pass(testCase, "Looking for camera named - "+inputs.getInputValue("LOCATION1_CAMERA1_NAME"));
		if(MobileUtils.isMobElementExists(fieldObjects, testCase, "Camera_Dasboard")){
			...
			if(!testCase.getPlatform().toUpperCase().contains("IOS")){
				if(MobileUtils.isMobElementExists("xpath", "//*[@text='"+inputs.getInputValue("LOCATION1_CAMERA1_NAME")+"']",testCase)){
					flag = flag & MobileUtils.clickOnElement(testCase, "xpath", "//*[@text='"+inputs.getInputValue("LOCATION1_CAMERA1_NAME")+"']");
				}else{
					Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE, "Camera not found");
					flag=false;
				}
			}else{
				if(MobileUtils.isMobElementExists("xpath", "//*[@value='"+inputs.getInputValue("LOCATION1_CAMERA1_NAME")+"']",testCase)){
					flag = flag & MobileUtils.clickOnElement(testCase, "xpath", "//*[@value='"+inputs.getInputValue("LOCATION1_CAMERA1_NAME")+"']");
				}else{
					Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE, "Camera not found");
					flag=false;
				}
	...
	public static boolean verifyLiveStreaming(TestCases testCase){
		boolean flag = true;
		fieldObjects = MobileUtils.loadObjectFile(testCase, "DAS_VideoSolution");
		DAS_Utils.waitForElementToDisappear(testCase, fieldObjects, "Progress_Message",50,5);
	...
	public static boolean VerifyAttentionAlarmScreen(TestCases testCase, TestCaseInputs inputs){
		...
		if(MobileUtils.isMobElementExists(fieldObjects, testCase, "Alarm_Title") && MobileUtils.isMobElementExists(fieldObjects, testCase, "AlarmDismissButton")){
	...
	public static boolean CreateAttention(TestCases testCase,TestCaseInputs inputs) {
		...
		try {
			...
			flag = false;
			Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE, "Could not create attention on camera solution card");
		}
		return flag;
	}
}
