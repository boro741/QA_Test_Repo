...
public class BaseStationSettingsScreen extends MobileScreens {
	...
	public boolean isEntryExitDelaySettingsOptionVisible() {
		return MobileUtils.isMobElementExists(objectDefinition, testCase, "EntryExitDelayOption", 5);
	}
	...
	public boolean clickSensorsOption() {
		return MobileUtils.clickOnElement(objectDefinition, testCase, "SensorsOption");
	}
	...
	public boolean selectOptionFromBaseStationSettings(String option) throws Exception {
		...
		case BaseStationSettingsScreen.BASESTATIONCONFIGURATION: {
			...
			if (this.isBaseStationConfigurationsOptionVisible()) {
				...
				System.out.println("Into IOS Loop");
...
//									BaseStationSettingsScreen.KEYFOB);
		...
		case BaseStationSettingsScreen.SENSORS: {
			...
			if (this.isSensorsOptionVisible()){
				flag = flag & this.clickSensorsOption();
			} else {
				if (testCase.getPlatform().toUpperCase().contains("ANDROID")) {
					int startx = (dimension.width * 20) / 100;
					int starty = (dimension.height * 62) / 100;
	...
	public boolean verifyBatteryOptionTextOnSensorSettingsScreen() {
		if (this.isSensorBatteryOptionVisibleOnSensorSettingsScreen()) {
			System.out.println(MobileUtils.getFieldValue(objectDefinition, testCase, "DASBatteryTitle"));
			return (MobileUtils.getFieldValue(objectDefinition, testCase, "DASBatteryTitle")
					.equalsIgnoreCase("Battery"));
		} else {
			return false;
	...
	public boolean isEnhancedDeterrenceSwitchEnabled(TestCases testCase) throws Exception {
		if (MobileUtils.isMobElementExists(objectDefinition, testCase, "EnhancedDeterrenceSwitch", 10)) {
			if (testCase.getPlatform().toUpperCase().contains("ANDROID")) {
				...
				return Boolean.logicalAnd(MobileUtils
						.getMobElement(objectDefinition, testCase, "EnhancedDeterrenceSwitch").getAttribute("value").equalsIgnoreCase(String.valueOf(1)),true);
			}
		} else {
			return false;
		}
	...
	public boolean isManageAlertExist() {
		return MobileUtils.isMobElementExists(objectDefinition, testCase, "ManageAlerts", 20);
	}
	...
	public boolean isEnhancedDeterrenceOptionVisible(int timeOut) {
		return MobileUtils.isMobElementExists(objectDefinition, testCase, "EnhancedDeterrence", timeOut);
	}

	public boolean clickOnEnhancedDeterrenceOption(int timeOut) {
		boolean flag = true;
		if (MobileUtils.isMobElementExists(objectDefinition, testCase, "EnhancedDeterrence", timeOut)) {
			flag &= MobileUtils.clickOnElement(objectDefinition, testCase, "EnhancedDeterrence");
		}
		return flag;
	}

	public boolean isEnhancedDeterrenceheadervisible() {
		return MobileUtils.isMobElementExists(objectDefinition, testCase, "EnhancedDeterrenceHeader", 20);
	}

	public boolean isEnhancedDeterrenceInsideDetererenceDescriptionvisible() {
		boolean flag = true;
		if (MobileUtils.isMobElementExists(objectDefinition, testCase, "EnhancedDeterrence", 20)) {
			flag &= MobileUtils.clickOnElement(objectDefinition, testCase, "EnhancedDeterrence");
			flag &= MobileUtils.isMobElementExists(objectDefinition, testCase,
					"EnhancedDeterrenceInsideDetererenceDescription", 20);
	...
	public boolean isEntryExitDelayEnabled() {
		if (testCase.getPlatform().toUpperCase().contains("ANDROID")) {
			...
			return MobileUtils.getFieldValue(objectDefinition, testCase, "EntryExitDelayOption")
					.equalsIgnoreCase("enabled");
		}
	}
	...
	public boolean isAboutSecurityModesEnabled() {
		if (testCase.getPlatform().toUpperCase().contains("ANDROID")) {
			...
			return MobileUtils.getFieldValue(objectDefinition, testCase, "AboutSecurityModes")
					.equalsIgnoreCase("enabled");
		}
	}
	...
	public boolean clickonAboutSecurityModesoption() throws InterruptedException {
		boolean flag = false;
