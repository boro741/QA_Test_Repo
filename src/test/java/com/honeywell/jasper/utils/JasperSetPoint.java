...
public class JasperSetPoint {
	...
	public static boolean rotateDialer(TestCases testCase, TestCaseInputs inputs, Double targetTemp) {
		...
		try {
			...
			if (targetTemp < minSetPoint || targetTemp > maxSetPoint) {
				Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE, "Setpoint Stepper : Target Temp : "
						+ targetTemp + " is out of range" + maxSetPoint + "-" + minSetPoint);
				return false;
			} else {
				...
				if (currentTemp != targetTemp) {
					...
					if (currentNewTemp == targetTemp) {
						Keyword.ReportStep_Pass(testCase,
								"Rotate Dialer : Successfully set dialer set points to : " + currentTemp);
						return flag;
					} else {
						flag = false;
						Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE,
								"Rotate Dialer : Failed to set dialer set points to: " + targetTemp);
					}
				}
			}
		} catch (Exception e) {
	...
	/**
	 * <p>
	 * The convertFromCelsiusToFahrenhiet method converts a temperature value from
	...
	public static String roundOffTimeToTheNearest1week(TestCases testCase, String time) {
		String roundOffTime = " ";
		try {
			SimpleDateFormat vacationDateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
			Calendar c = Calendar.getInstance();
	...
	public static String roundOffTimeToUpcomingNearest15minutes(TestCases testCase, String time) {
		String roundOffTime = " ";
		try {
			SimpleDateFormat vacationDateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
			Calendar c = Calendar.getInstance();
	...
	public static String roundOffTimeToUpcomingNearest10minutes(TestCases testCase, String time) {
		String roundOffTime = " ";
		try {
			SimpleDateFormat vacationDateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
			Calendar c = Calendar.getInstance();
	...
	public static String CalculatePeriodEndEMEA(TestCases testCase, int hour) {
		String formattedTime = "";
		try {
	...
	public static String CalculatePreviosPeriodStartEMEAHB(TestCases testCase) {
		String formattedTime = " ";
		try {
			Calendar startTime = Calendar.getInstance();
			int minutes = startTime.get(Calendar.MINUTE);
			int mod = minutes % 15;
			startTime.add(Calendar.MINUTE, -10 - mod);
			startTime.set(Calendar.SECOND, 0);
			Date date = startTime.getTime();
			DateFormat SleepStart = new SimpleDateFormat("HH:mm:ss");
			formattedTime = SleepStart.format(date);
	...
	public static String CalculatePeriodEndNAHB(TestCases testCase, int hour) {
		String formattedTime = "";
		try {
			Calendar EndTime = Calendar.getInstance();
			int minutes = EndTime.get(Calendar.MINUTE);
	...
	public static String CalculateNextPeriodStartNAHB(TestCases testCase) {
		String formattedTime = " ";
		try {
			Calendar startTime = Calendar.getInstance();
			int minutes = startTime.get(Calendar.MINUTE);
	...
	public static String CalculateNextPeriodStartEMEA(TestCases testCase) {
		String formattedTime = "";
		try {
			Calendar EndTime = Calendar.getInstance();
			int minutes = EndTime.get(Calendar.MINUTE);
			int mod = minutes % 10;
			int diff = 10 - mod;
			EndTime.add(Calendar.MINUTE, diff);
			EndTime.set(Calendar.SECOND, 0);
			Date date = EndTime.getTime();
			DateFormat dateFormat = new SimpleDateFormat("HH:mm:ss");
			formattedTime = dateFormat.format(date);
	 ...
	 * @param TestCases
	 * @param String
	 *            - This is a date in the format 'yyyy-MM-dd'T'HH:mm:ss' to which
	 *            days have to be added.
	 * @param Integer
	 *            - This is the number of days to be added to the data passed in the
	 *            above parameter.
