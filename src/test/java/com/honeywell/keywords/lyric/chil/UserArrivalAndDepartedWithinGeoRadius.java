...
public class UserArrivalAndDepartedWithinGeoRadius extends Keyword {
	...
	public UserArrivalAndDepartedWithinGeoRadius(TestCases testCase, TestCaseInputs inputs,
		...
		try {
			...
			if (parameters.get(0).equalsIgnoreCase("home")) {
				...
				if (chUtil.getConnection()) {
					int result = chUtil.userArrived(chUtil.getLocationID(inputs.getInputValue("LOCATION1_NAME")),
							chUtil.getUserID(), geofenceID);

					Thread.sleep(3000);

					result = chUtil.userDeparted(chUtil.getLocationID(inputs.getInputValue("LOCATION1_NAME")),
							chUtil.getUserID(), geofenceID);
					Thread.sleep(2000);
				...
				if (chUtil.getConnection()) {
					int result = chUtil.userDeparted(chUtil.getLocationID(inputs.getInputValue("LOCATION1_NAME")),
							chUtil.getUserID(), geofenceID);
					Thread.sleep(10000);
					result = chUtil.userArrived(chUtil.getLocationID(inputs.getInputValue("LOCATION1_NAME")),
							chUtil.getUserID(), geofenceID);
					if (result == HttpURLConnection.HTTP_OK) {
