...
public class LocationsSortingOrderInLocationsDropDownListWhenALocationIsSelected extends Keyword {
	...
	public LocationsSortingOrderInLocationsDropDownListWhenALocationIsSelected(TestCases testCase, TestCaseInputs inputs,
	...
	@Override
	@KeywordStep(gherkins = "^list of \"(.+)\" should be sorted in the following alphanumeric order when a location is selected:$")
	public boolean keywordSteps() throws KeywordException {
		Dashboard d = new Dashboard(testCase);
		ArrayList<String> locationsListDisplayedAfterSelectingALocation = new ArrayList<String>();
		if (testCase.isTestSuccessful() || VerifyIfDefaultLocationDisplayedInDashboardScreenAfterLogin.FLAG) {
			...
			for (int i = 0; i < correctSortingOrderForExpectedLocations.size(); i++) {
				if ((!inputs.getInputValue("SELECTED_LOCATION_IN_DASHBOARD").isEmpty()
			...
			if (d.isLocationListDisplayedWhenClickedOnLocationDropDownArrow()) {
				locationsListDisplayedAfterSelectingALocation
			...
			for (int i = 0; i < correctSortingOrderForExpectedLocations.size(); i++) {
				...
				} else if (locationsListDisplayedAfterSelectingALocation.get(i)
						.equals(correctSortingOrderForExpectedLocations.get(i))) {
					Keyword.ReportStep_Pass(testCase,
							"Expected Location: " + correctSortingOrderForExpectedLocations.get(i)
