...
public class CreateDefaultTimeBasedScheduleByDeletingMultiplePeriods extends Keyword {
	...
	public CreateDefaultTimeBasedScheduleByDeletingMultiplePeriods(TestCases testCase, TestCaseInputs inputs,
	...
	@Override
	@KeywordStep(gherkins = "^user creates \"(.+)\" schedule by deleting \"(.+)\" from the default schedule values$")
	public boolean keywordSteps() throws KeywordException {
		try
		{
			...
			if (exampleData.get(1).equalsIgnoreCase("Atleast 1 period")) {
				inputs.setInputValue(InputVariables.NUMBER_OF_PERIODS_TO_DELETE, "ONE");
			} else if (exampleData.get(1).equalsIgnoreCase("Atleast 2 period")) {
			...
			if (exampleData.get(0).equalsIgnoreCase("Same Every Day")) {
				...
				inputs.setInputValue(InputVariables.WEEKEND_4_TIME, defaultValues.get("WeekendSleepTime").toLowerCase().replaceAll("^0*", ""));
				inputs.setInputValue(InputVariables.WEEKEND_SLEEP_TIME, defaultValues.get("WeekendSleepTime").toLowerCase().replaceAll("^0*", ""));
