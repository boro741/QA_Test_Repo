			...
			if (statInfo.getThermostatType().equals("Jasper")) {
				...
				if (testCase.getPlatform().toUpperCase().contains("ANDROID")) {
					for (int j = 1; j <= i; j++) {
						...
						flag = flag & JasperSchedulingEditUtils.editTimeBasedScheduleByDeletingPeriods(testCase, inputs,
						...
						do {
							if (inputs.getInputValue(InputVariables.SHOW_VIEW_TYPE).equalsIgnoreCase("Grouped Days")) {
								...
								schedulePeriodToSelect = days[rn.nextInt((6 - 0) + 1) + 0] + "_"
										+ schedulePeriods[rn.nextInt((3 - 0) + 1) + 0];
							}
						} while (arrlist.contains(schedulePeriodToSelect));
						arrlist.add(schedulePeriodToSelect);
				...
				// //Commenting the code to check it doesn't break any other scenarios
			}
		} catch (Exception e) {
