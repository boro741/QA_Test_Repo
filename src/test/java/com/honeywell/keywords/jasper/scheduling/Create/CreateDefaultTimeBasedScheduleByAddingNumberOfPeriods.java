...
public class CreateDefaultTimeBasedScheduleByAddingNumberOfPeriods extends Keyword {
	...
	public CreateDefaultTimeBasedScheduleByAddingNumberOfPeriods(TestCases testCase, TestCaseInputs inputs,
	...
	@Override
	@KeywordStep(gherkins = "^user creates \"(.+)\" schedule by adding \"(.+)\" period to the default schedule values$")
		...
		try
		{
			...
			if (exampleData.get(0).equalsIgnoreCase("Same Every Day")) {
				...
				if (jasperStatType.equalsIgnoreCase("EMEA")) {
					inputs.setInputValue(InputVariables.ADD_PERIOD, "Yes");
					inputs.setInputValueWithoutTarget("ADD_PERIOD_NUMBER", exampleData.get(1));
					inputs.setInputValue(InputVariables.DELETE_PERIOD, "No");
					temp = JasperSchedulingUtils.getRandomSetPointValueBetweenMinandMax(testCase, inputs, maxHeat, minHeat);
					inputs.setInputValue(InputVariables.ADD_PERIOD_HEAT_SETPOINT, temp);
					if(inputs.getInputValue("ADD_PERIOD_NUMBER").equals("6")){
						temp = JasperSchedulingUtils.getRandomSetPointValueBetweenMinandMax(testCase, inputs, maxHeat, minHeat);
						inputs.setInputValue("ADD_PERIOD_6_HEAT_SETPOINT", temp);
					}
				}
