...
@DAS_MotionSensorStatusVerification 
...
#Requirement :One DAS Panel and one motion sensor should be configured
@DAS_MotionSensorRenamePopUpVerification 
...
@DASmotion sensorStatusVerificationoffline
  Scenario: As a user I want to view that all motion sensor settings 
    Given user launches and logs in to the Lyric application 
     When user navigates to "Motion Sensor settings" screen from the "Dashboard" screen 
...
#Requirement :One DAS Panel and one motion sensor should be configured
@DASmotion sensorSensorStatusVerificationWithHomeAwayNightOffMode
...
#Requirement :One DAS Panel and one motion sensor should be configured
@DASmotion sensorSensorStatusOFFVerificationWithHomeAwayNightOffMode
Scenario Outline:AS a user I want to Verify motion sensor Battery status OFF 
Given user launches and logs in to the Lyric application 
And user is set to <Mode> through CHIL
...
Then user should navigates to " motion sensor OFF" screen
Examples:
|Mode | 
|Away |
...
@DASmotion sensorStatusCoverTamperedVerificationHome
Scenario Outline:AS a user I want to Verify motion sensor Cover Tamper and Tamper restored status When base station status in Home and OFF mode
Given user launches and logs in to the Lyric application 
And user is set to <Mode> through CHIL
#motion sensor back plate not closed
Then user navigates to "Motion Sensor settings" screen from the "Dashboard"
When user selects "Status Covered Tampered" 
Then user should be displayed with "motion sensor Cover Tampered" screen
...
Then user should displayed with "motion sensor Cover Tampered" screen
And User selects the "Clear Tamper" option 
Then user should be displayed with "Sensor Tamper" pop up 
...
And User selects the "Clear Tamper" option 
Then user should be displayed with "Sensor Tamper" pop up 
#motion sensor back plate closed
When user selects the "RETRY" button
Then user should navigates to "Motion Sensor Settings" screen
...
#Requirement :One DAS Panel and one motion sensor should be configured and motion sensor status cover tampered status
@DASmotion sensorStatusCoverTamperedVerificationAwayNightoff
...
@DASmotion sensorStatusCoverTamperedVerificationHomeawaynightoff
Scenario Outline:AS a user I want to Verify motion sensor Cover Taper status When base station status in Away, Night, OFF mode
Given user launches and logs in to the Lyric application 
And user is set to <Mode> through CHIL
...
#Requirement :One DAS Panel and one motion sensor should be configured and motion sensor status cover tampered status
@DASmotion sensorStatusCoverTamperedVerificationOffline
...
#Requirement :One DAS Panel and one motion sensor should be configured
@DASmotion sensorBatteryStatusVerificationWithHomeAwayNightOffOfflineMode
Scenario Outline:AS a user I want to Verify motion sensor Battery status
Given user launches and logs in to the Lyric application 
And user is set to <Mode> through CHIL
Then user navigates to "Motion Sensor settings" screen from the "Dashboard"
...
@DASmotion sensorBatteryLowofflineStatusLowHelpScreenVerification
...
#Requirement :One DAS Panel and one motion sensor should be configured
@DASmotion sensorBatteryStatusWhenSensorstatusOfflineVerification
Scenario Outline:AS a user I want to Verify motion sensor Battery status
Given user launches and logs in to the Lyric application 
And user is set to <Mode> through CHIL
Then user navigates to "Motion Sensor settings" screen from the "Dashboard"
...
Then user should see the "motion sensor" status as <Motion Status Update> on the "Test Access Sensor"
Examples:
...
@DASmotion sensorEnrollmentWithSensorNotWorkingAndIsOutOfRange
...
#Requirement :One DAS Panel and one motion sensor should be configured
@DASmotion sensorSignalStrengthRangeTestVerificationSensorOffline
...
Scenario Outline: i -As a user I should be able to delete motion sensor configured to my DAS panel from my account through the Lyric application 
Given user launches and logs in to the Lyric application
When user navigates to "Motion Sensor Settings" screen from the "DASHBOARD" screen
...
#Requirement :One DAS Panel and one motion sensor should be configured
@DASDeletemotion sensorErrorpopupVerification
...
#Requirement :One DAS Panel and one motion sensor should be configured
@DASDeletemotion sensorPopupVerification @UIAutomated 
Scenario Outline: As a user I should be able to delete motion sensor configured to my DAS panel from my account through the Lyric application 
Given user launches and logs in to the Lyric application
And user is set to <Mode> through CHIL 
When user navigates to "Motion Sensor settings" screen from the "Dashboard" screen 
And user "deletes motion sensor" by clicking on "delete" button
