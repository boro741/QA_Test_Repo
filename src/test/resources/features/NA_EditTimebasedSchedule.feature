...
@ViewTimeschedulePrimarycardNA @Automated @--xrayid:ATER-7585
  ...
  @ViewTimescheduleIndividualdaysNA @Automated @--xrayid:ATER-7587
  ...
  @EditTimescheduleTimeformatNA @Automated @--xrayid:ATER-7590
  ...
  Scenario Outline: The Time schedule removed if setting up with time value as same as it for new time schedule for systems Heat cool,Cool,Heat for Temperature scale Celsius or Fahrenheit and for time format 24 or 12hr
    ...
    When user selects schedule icon from primary card
    And user selects view by <Type>
     And user edit "Wake" period by changing time value to "06 00 AM"
     When user edit "Away" period by changing time value to "06 00 AM"
     Then verify "Time" schedule successfully gets edited 
      And the "Wake" period time value is displayed as "Tap to set"
      And user logs out of the app
      Examples:
      | Type            | 
      | Grouped days    | 
  ...
  Scenario Outline: To edit Time schedule by deleting period for systems Heat cool,Cool,Heat for Temperature scale Celsius or Fahrenheit and for time format 24 or 12hr
    ...
    When user selects schedule icon from primary card
    And user selects view by <Type>
     When user edit Time schedule by deleting <Period> on confirming the period deletion
     Then verify "Time" schedule successfully gets edited
     And user logs out of the app
    Examples: 
      | Type            | Period           | 
