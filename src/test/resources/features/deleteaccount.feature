...
\f0\fs22 \cf2 \cb3 Feature: As an user, I want to delete my account from the app. @LYR-22282\
\
\
...
@SingleUserWithUnsharedDeviceWithCameraSubscriptions.\
	Scenario: To verify user is not able to delete his account when he has a unshared device or a Camera Subscription in it\
	Given app is launched\
...
Scenario: To verify all logged in devices should log out if account is deleted.\
...
Scenario: To verify user is navigated to add device screen when a shared location is deleted and if account is deleted.\
	Given app is launched in two devices\
...
Scenario: To verify invited user is able to access the primary user shared location and device even if primary user deleted its account.\
	Given app is launched in two devices\
		And both users taps on LOGIN in with different accounts\
		And both users should have <DEVICE> in one LOCATION\
