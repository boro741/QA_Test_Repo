...
@RFDoorsensor_InAwayExitDelay_OpenDoor_NoAlarm 				@P2 				@KATANA_RF_DoorSensor 		@--xrayid:ATER-99027			@Automated
  Scenario: As a user when I open the RF door Sesnor during exit delay i should not get alarm  
   ...
   #Given user is set to "Away" mode through CHIL
    ...
    Scenario: As a user when I open the RF door and left open in armed away state on my arrival to home I should be able to switch to home from entry delay screen and should be shown with current door status
    Given user launches and logs in to the Lyric application
    And user is set to "Away" mode through CHIL
      #And user clears all push notifications
     When user navigates to "Security Solution card" screen from the "Dashboard" screen
    ...
    Scenario: As a user when I open the RF door in armed away on my arrival and I should be able to switch to home from pn 
    Given user is set to "Away" mode through CHIL
    Given user launches and logs in to the Lyric application
      And user clears all push notifications
     When user navigates to "Security Solution card" screen from the "Dashboard" screen
...
@RFDoorsensor_ArmedAway_OpenDoor_SwitchingToNightFromPushNotification_DoorNotClosedInEntryDelay_Alarm  @--xrayid:ATER-99935			@P2			@Automated		 #N			 @DAS_DoorSensor
  ...
  Then verify the following activity log:
   | Elements                 |
    ...
    | RF Door Closed at Away mode |
     ...
     @RFDoorsensor_ArmedAway_OpenDoor_SwitchingToNightFromEntryDelay_DoorOpenedInEntryDelay_NoAlarm 		@--xrayid:ATER-			@P3 			#N 		@DAS_DoorSensor 			@Automated
    ...
    Given user sets the entry/exit timer to "60" seconds
     ...
     When user "RF door Sensor" access sensor "Tampered"
      ...
      And user navigates to "Security Solution card" screen from the "Dashboard" screen
      When user navigates to "Sensor Status" screen from the "Security Solution Card" screen
      When user "RF door sensor" access sensor "Tamper Restored"
